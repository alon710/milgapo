...
export function OTPInput({ length, value, onChange }: OTPInputProps) {
    ...
    return (
        <div className="flex justify-center gap-2 ltr" dir="ltr">
            {otp.map((digit, index) => (
                <div key={index} className="w-11 h-11 md:w-12 md:h-12 relative">
                    <Input
                        ...
                        onBlur={handleBlur}
                        className={`
                            text-center text-xl font-medium h-11 md:h-12 w-11 md:w-12 transition-all duration-300
                            ${activeInput === index ? "ring-2 ring-primary ring-offset-2" : ""}
                            ${digit ? "bg-primary/5 border-primary/30" : ""}
                        `}
                        autoFocus={index === 0 && !digit}
                    />
                    {index < length - 1 && <div className="absolute top-1/2 -right-2 w-2 h-[2px] bg-gray-300" />}
                </div>
            ))}
        </div>
    );
